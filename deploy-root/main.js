...
const SERVICE_API_URL = "https://script.google.com/macros/s/AKfycbztc3aEoq5J1_JuMeWLiwrLDgaVy5EiJWfRXiWWB3jzJLad5_rkHwTQC4z3ry8YGZ5OYg/exec";
const SERVICE_SESSION_KEY = "gjstrings.serviceSession";
...
const qrChoiceDialog = document.querySelector("#qrChoiceDialog");
const qrChoiceDetailButton = document.querySelector("#qrChoiceDetailButton");
const qrChoiceLoginButton = document.querySelector("#qrChoiceLoginButton");
...
const serviceUserName = document.querySelector("#serviceUserName");
const logoutButton = document.querySelector("#logoutButton");
const serviceScanButton = document.querySelector("#serviceScanButton");
const openRacketFormButton = document.querySelector("#openRacketFormButton");
const openStringingFormButton = document.querySelector("#openStringingFormButton");
const openStringFormButton = document.querySelector("#openStringFormButton");
const racketDialog = document.querySelector("#racketDialog");
const racketForm = document.querySelector("#racketForm");
const closeRacketFormButton = document.querySelector("#closeRacketFormButton");
const stringingDialog = document.querySelector("#stringingDialog");
const stringingForm = document.querySelector("#stringingForm");
const closeStringingFormButton = document.querySelector("#closeStringingFormButton");
const stringDialog = document.querySelector("#stringDialog");
const stringForm = document.querySelector("#stringForm");
const closeStringFormButton = document.querySelector("#closeStringFormButton");
...
function showQrChoiceDialog(code) {
  state.pendingRacketCode = code;
  qrChoiceDialog.hidden = false;
}

function hideQrChoiceDialog() {
  qrChoiceDialog.hidden = true;
}
...
function restoreServiceSession() {
  try {
    const session = JSON.parse(localStorage.getItem(SERVICE_SESSION_KEY) || "{}");
    state.serviceToken = session.token || "";
    state.serviceUser = session.user || null;
  } catch (error) {
...
function saveServiceSession(data) {
  state.serviceToken = data.token || "";
  state.serviceUser = data.user || null;
  localStorage.setItem(SERVICE_SESSION_KEY, JSON.stringify({
    token: state.serviceToken,
    user: state.serviceUser
  }));
  renderServiceState();
  if (state.pendingRacketCode) {
    const code = state.pendingRacketCode;
    state.pendingRacketCode = "";
    openRacketDetail(code);
  }
}
...
function isServiceLoggedIn() {
  return Boolean(state.serviceToken && state.serviceUser);
}

function renderServiceState() {
  const loggedIn = isServiceLoggedIn();
  if (loginForm) loginForm.hidden = loggedIn;
  if (servicePanel) servicePanel.hidden = !loggedIn;
  if (serviceUserName) serviceUserName.textContent = state.serviceUser?.name || "";
  document.querySelectorAll(".service-only").forEach((node) => {
...
async function callServiceApi(action, data = {}) {
  ...
  if (!result.ok) throw new Error(result.error || "Akce se nepodařila.");
  return result;
}

async function reloadDataAfterServiceAction() {
  const data = await loadData();
  state.rows = data.rows;
  state.inventory = data.inventory;
  if (state.selectedRacketCode) {
    openRacketDetail(state.selectedRacketCode);
  } else {
    render();
  }
}

function showStringingDialog() {
  ...
  const dateInput = stringingForm?.elements.date;
  if (dateInput && !dateInput.value) dateInput.value = formatDate(new Date());
  stringingDialog.hidden = false;
}

function showRacketDialog() {
  if (!isServiceLoggedIn()) {
    setScreen("serviceScreen");
    return;
  }
  const ownerInput = racketForm?.elements.owner;
  if (ownerInput && state.owner && !ownerInput.value) ownerInput.value = state.owner;
  racketDialog.hidden = false;
}

function hideRacketDialog() {
  racketDialog.hidden = true;
}

function hideStringingDialog() {
  stringingDialog.hidden = true;
}

function showStringDialog() {
  if (!isServiceLoggedIn()) {
    setScreen("serviceScreen");
    return;
  }
  stringDialog.hidden = false;
}

function hideStringDialog() {
  stringDialog.hidden = true;
}
...
function renderDetail(history) {
  ...
  const serviceNotice = Number.isFinite(daysSinceLastStringing) && daysSinceLastStringing > OLD_STRINGING_DAYS
    ...
    : "";
  const serviceCode = isServiceLoggedIn()
    ? `<div class="spec"><span>Kód rakety</span><strong>${escapeHtml(racket.racketCode || "-")}</strong></div>`
    : "";
...
qrChoiceDetailButton.addEventListener("click", () => {
  const code = state.pendingRacketCode;
  state.pendingRacketCode = "";
  hideQrChoiceDialog();
  openRacketDetail(code);
});
qrChoiceLoginButton.addEventListener("click", () => {
  hideQrChoiceDialog();
  setScreen("serviceScreen");
});
qrChoiceDialog.addEventListener("click", (event) => {
  if (event.target === qrChoiceDialog) {
    hideQrChoiceDialog();
    state.pendingRacketCode = "";
  }
});
...
loginForm.addEventListener("submit", async (event) => {
  ...
  try {
    const result = await callServiceApi("login", {
      email: formData.get("email"),
      password: formData.get("password")
    });
    saveServiceSession(result);
    loginForm.reset();
  } catch (error) {
...
logoutButton.addEventListener("click", clearServiceSession);
serviceScanButton.addEventListener("click", startScan);
openRacketFormButton.addEventListener("click", showRacketDialog);
openStringingFormButton.addEventListener("click", showStringingDialog);
openStringFormButton.addEventListener("click", showStringDialog);
closeRacketFormButton.addEventListener("click", hideRacketDialog);
closeStringingFormButton.addEventListener("click", hideStringingDialog);
closeStringFormButton.addEventListener("click", hideStringDialog);
if (racketDialog) {
  racketDialog.addEventListener("click", (event) => {
    if (event.target === racketDialog) hideRacketDialog();
  });
}
stringingDialog.addEventListener("click", (event) => {
  if (event.target === stringingDialog) hideStringingDialog();
});
stringDialog.addEventListener("click", (event) => {
  if (event.target === stringDialog) hideStringDialog();
});

stringingForm.addEventListener("submit", async (event) => {
  event.preventDefault();
  const formData = new FormData(stringingForm);
  try {
    await callServiceApi("addStringing", {
      token: state.serviceToken,
      racketCode: state.selectedRacketCode,
      stringCode: formData.get("stringCode"),
      tension: formData.get("tension"),
      date: formData.get("date")
    });
    hideStringingDialog();
    stringingForm.reset();
    await reloadDataAfterServiceAction();
  } catch (error) {
    window.alert(error.message);
  }
});

racketForm.addEventListener("submit", async (event) => {
  event.preventDefault();
  const formData = new FormData(racketForm);
  try {
    await callServiceApi("addRacket", {
      token: state.serviceToken,
      code: formData.get("code"),
      name: formData.get("name"),
      owner: formData.get("owner"),
      stringLength: formData.get("stringLength"),
      knots: formData.get("knots")
    });
    hideRacketDialog();
    const newCode = String(formData.get("code") || "");
    racketForm.reset();
    await reloadDataAfterServiceAction();
    if (newCode) openRacketDetail(newCode);
  } catch (error) {
    window.alert(error.message);
  }
});
...
stringForm.addEventListener("submit", async (event) => {
  ...
  const owner = ownerMode === "customer" ? state.owner : state.serviceUser?.name;
  try {
    await callServiceApi("addString", {
      token: state.serviceToken,
      code: formData.get("code"),
      name: formData.get("name"),
...
async function boot() {
  ...
  if (scannedCode) {
    if (isServiceLoggedIn()) {
      ...
      render();
      showQrChoiceDialog(scannedCode);
    }
  } else {
    render();
    setScreen("homeScreen");
